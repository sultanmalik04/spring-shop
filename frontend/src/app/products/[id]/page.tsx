...
const ProductDetailPage = () => {
  ...
  const { isAdmin, isAuthenticated, isLoading } = useAuth(); // Use useAuth hook
    ...
    <div className="container mx-auto p-4 flex flex-col md:flex-row items-center md:items-start gap-8">
      ...
      <div className="md:w-1/2 w-full bg-white p-6 rounded-lg shadow-md">
        ...
        {isAuthenticated && isAdmin ? (
          <div className="mt-4">
            <Link href={`/admin/products/${product.id}/edit`} className="bg-yellow-500 hover:bg-yellow-600 text-white font-bold py-3 px-6 rounded-lg text-lg transition-colors duration-300">
              Edit Product
            </Link>
          </div>
        ) : (
